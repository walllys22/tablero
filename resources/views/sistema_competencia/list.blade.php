                ...
                @forelse ($data as $item)
                    <tr>
                        ...
                        <td style="vertical-align: middle; width: 16%" class="text-end">
                            ...
                            @else
                                <form method="POST" action="{{ route('sistema-competencia.toggle-status', $item) }}" class="d-inline">
                                    @csrf
                                    @method('PATCH')
                            ...
                            <a href="#" onclick="event.preventDefault(); deleteItem('{{ route('sistema-competencia.destroy', $item) }}')" title="Eliminar" data-bs-toggle="modal" data-bs-target="#modal-delete" class="btn btn-sm btn-danger">
...
@foreach ($data as $item)
    <div class="modal fade" id="modal-status-{{ $item->id }}" tabindex="-1" aria-labelledby="modalStatusLabel{{ $item->id }}" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <form method="POST" action="{{ route('sistema-competencia.toggle-status', $item) }}">
                @csrf
                @method('PATCH')
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                <div class="modal-header py-2 bg-primary text-white">
                    <h5 class="modal-title fw-bold" id="modalViewLabel{{ $item->id }}">Datos de sistema de competencia</h5>
    ...
    <div class="modal fade" id="modal-edit-{{ $item->id }}" tabindex="-1" aria-labelledby="modalEditLabel{{ $item->id }}" aria-hidden="true">
        <div class="modal-dialog">
            <form method="POST" action="{{ route('sistema-competencia.update', $item) }}">
                @csrf
                @method('PATCH')
                <input type="hidden" name="editing_sistema_competencia" value="{{ $item->id }}">

                <div class="modal-content">
                    <div class="modal-header bg-info text-dark">
                        <h5 class="modal-title fw-bold" id="modalEditLabel{{ $item->id }}">Editar sistema de competencia</h5>
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            <div class="col-md-8">
                                ...
                                <input type="text" name="nombre" id="nombre_edit_{{ $item->id }}" value="{{ old('editing_sistema_competencia') == $item->id ? old('nombre') : $item->nombre }}" class="form-control @if(old('editing_sistema_competencia') == $item->id) @error('nombre') is-invalid @enderror @endif" maxlength="255" required>
                                @if (old('editing_sistema_competencia') == $item->id)
                                    @error('nombre')
                            ...
                            <div class="col-md-4">
                                ...
                                @php
                                    $estadoActual = old('editing_sistema_competencia') == $item->id ? old('estado') : $item->estado;
                                @endphp
                                <select name="estado" id="estado_edit_{{ $item->id }}" class="form-select @if(old('editing_sistema_competencia') == $item->id) @error('estado') is-invalid @enderror @endif" required>
...
<script>
    $(document).ready(function () {
        ...
        @if ($errors->any() && old('editing_sistema_competencia'))
            let editModal = document.getElementById('modal-edit-{{ old('editing_sistema_competencia') }}');
            if (editModal) {
