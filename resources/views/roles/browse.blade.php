            ...
            <form method="POST" action="{{ route('roles.store') }}">
                ...
                <div class="modal-content">
                    <div class="modal-header bg-success text-white">
                        <h5 class="modal-title fw-bold" id="modalCreateLabel">Crear rol</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="name" class="form-label">Nombre</label>
                            <input type="text" name="name" id="name" value="{{ old('name') }}" class="form-control @if(!old('editing_role')) @error('name') is-invalid @enderror @endif" maxlength="255" required>
                            @if (!old('editing_role'))
                                @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="description" id="description" value="{{ old('description') }}" class="form-control @if(!old('editing_role')) @error('description') is-invalid @enderror @endif" maxlength="255">
                            @if (!old('editing_role'))
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
